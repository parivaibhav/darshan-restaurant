...
if (empty($_POST['confirm_phrase']) || trim($_POST['confirm_phrase']) !== 'DELETE') {
    header('Location:/college/users/index?error=confirm');
    exit();
}

$email = $user['email'] ?? '';
if (!$email) {
    header('Location: /college/users/index?error=noemail');
    exit();
}
...
try {
    ...
    header("Location: /college/users/index?error=deletion&details=$err");
    exit();
}
...
header('Location: /college/login?account=deleted');
exit();
